...
//   const allUsers = await userService.getUserById(userId);
...
const getUser = catchAsync(async (req, res) => {
  ...
  // Retrieve user data from MongoDB based on the provided user ID
  ...
  // If the query parameter is 'address', return only the address field
  if (queryParam === 'address') {
    // return res.status(200).json({ address: user.address });
    const addressDetails = await userService.getUserAddressById(req.params.userId);
    res.send({ address: addressDetails.address });
  }

  // If the query parameter is not 'address', return the entire user object
  res.send(user);
});
